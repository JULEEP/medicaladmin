...
const CancelledOrders = () => {
  ...
  // Fetch cancelled orders when the component mounts
  useEffect(() => {
  ...
  const handleSaveStatus = async () => {
    if (selectedOrder) {
      try {
        ...
        // Remove the order from cancelled orders list if status is no longer "Cancelled"
        if (newStatus !== "Cancelled") {
          setOrdersData(ordersData.filter((order) => order._id !== selectedOrder._id));
        } else {
  ...
  const handleRestoreOrder = async (orderId) => {
    if (window.confirm("Are you sure you want to restore this order?")) {
      try {
        ...
        // Remove the order from cancelled orders list
  ...
  if (loading) return (
    <div className="flex justify-center items-center h-64">
      <p className="text-lg text-blue-900">Loading cancelled orders...</p>
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-white shadow-lg rounded-lg">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-red-900">Cancelled Orders</h2>
        <div className="bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-medium">
          Total: {ordersData.length} cancelled orders
        </div>
      </div>
          ...
          <table className="w-full border-collapse border border-gray-300">
            <thead className="bg-red-600 text-white">
              <tr>
                ...
                <th className="p-3 border text-left">Cancelled Date</th>
                <th className="p-3 border text-left">Actions</th>
              </tr>
            </thead>
            <tbody>
              {ordersData.map((order, index) => (
                <tr key={order._id} className="border-b hover:bg-red-50 transition-colors">
          ...
          <div className="bg-white p-6 rounded-lg w-11/12 md:w-1/3 max-w-md">
            ...
            <div className="flex justify-between gap-4 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors flex-1"
              >
                Save Changes
              </button>
            </div>
